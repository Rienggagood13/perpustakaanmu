                    ...
                    <tbody>
                        <?php $__currentLoopData = $pesanan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="text-center">
                                ...
                                <?php elseif($row->status == 2): ?>
                                    <td> <span class="badge badge-danger">Konfirmasi Pembayaran</span>
                                        <?php if(auth()->user()->role == 1): ?>
                                            <a href="javascript:;" data-image="<?php echo e($row->bukti_bayar); ?>" data-id="<?php echo e($row->order_no); ?>" data-toggle="modal" data-target="#imgPreview" class="btn btn-sm btn-style rounded-circle   modal-image">
                                ...
                                <td>
                                    <?php if(auth()->user()->role == 1): ?>
                                    <a href="<?php echo e(route('admin.detpes', $row->no_pesanan)); ?>"
                                    class="btn btn-primary btn-sm"><i class="fas fa-fw fa-search"></i></i></a>
                                    <?php if($row->status == 2): ?>
                                    <form action="<?php echo e(route('konfirmasi_pembayaran', $row->no_pesanan)); ?>" method="POST" class="d-inline">
                                    ...
                                    <?php endif; ?>
                                    <?php else: ?>
                                    <a href="<?php echo e(route('seller.detpes', $row->no_pesanan)); ?>"
                                    class="btn btn-primary btn-sm"><i class="fas fa-fw fa-search"></i></i></a>
                                        <?php if($row->status == 3): ?>
                                        <a href="#" data-id="<?php echo e($row->id); ?>" data-invoice="<?php echo e($row->no_pesanan); ?>" data-toggle="modal" data-target="#invoice" class="btn btn-sm btn-primary modalInvoice"> <i class="fas fa-fw fa-caravan"></i></a>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </td>
                            </tr>
...
<script>
    $(document).on("click", ".modal-image", function() {
        ...
        var imgsrc = '<?php echo e(asset("storage")); ?>' + '/' + img;
...
<?php echo $__env->make('layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\klontong_kasep\resources\views/pesanan/index.blade.php ENDPATH**/ ?>